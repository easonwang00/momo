...
export default class FlowDetail extends Component {
  ...
  getContentHotWander = async (content, cIndex) => {
    if (content["hot_wander"] && content["hot_wander"].length) return;
    const { data } = this.state;
    content["loading_hot_wander"] = true;
    this.setState(
      {
        data: {
          ...data,
      ...
      async () => {
        ...
        content["loading_hot_wander"] = false;
  ...
  onTxtSelect = async (event, cIndex, txt, content, type) => {
    ...
    if (selectTxt != txt) {
      ...
      if (content["creat_wander"] && content["creat_wander"] != " ") {
        const temp = data.contents;
  ...
  render() {
    ...
    const tempContributors = contributors ? contributors.slice(0, 3) : [];
    return (
      <ScrollView
                ...
                <View style={[styles.containerStyle]}>
                  ...
                  <View style={{ overflow: "hidden" }}>
                    {obj.primary_card.map(c => (
                      <Text style={{ fontSize: 16, fontWeight: 400, lineHeight: 24 }} key={c}>
                        {c}
                      </Text>
                    ))}
                  </View>
                ...
                {c.hot_wander && c.hot_wander.length ? (
                  <View>
                    <EZSwiper
                      ...
                      dataSource={c.hot_wander}
                      width={c.hot_wander.length == 1 ? 340 : 320}
                      height={160}
                      loop={false}
                      offset={12}
                      // ratio={0.95}
                      renderRow={obj => (
                        <View
                          style={[
                            ...
                            {
                              width: c.hot_wander.length == 1 ? 340 : 320,
                              marginLeft: c.hot_wander.length == 1 ? 14 : 24,
                            },
                          ]}
                        >
                          <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  <>
                    <View style={styles.wanderCard} onLayout={this._onLayout}>
                      ...
                      <View style={styles.body}>
                        {!c.creat_wander || c.creat_wander == " " ? (
                          <img src={config.loadingPrompts} width="100%" height="100%" />
                        ) : null}
                      ...
                      <View style={styles.footer}>
                        ...
                        >
                          <Text style={styles.footerBtnTxt}>Nope, try again {nopeClick}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                        ...
                        >
                          <Text style={styles.footerBtnTxt}>Awesome {aweSomeClick}</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
